...
def hwp_to_markdown(hwp_path: str) -> str:
    """HWP 파일을 마크다운으로 변환합니다. (converters.convert_to_markdown 위임)"""
    from .converters import convert_to_markdown

    return convert_to_markdown(hwp_path)


def hwp_to_md(hwp_path: str) -> str:
    return hwp_to_markdown(hwp_path)


def hwp_to_pdf(hwp_path: str, output_pdf_path: Optional[str] = None) -> str:
    """HWP 파일을 PDF로 변환합니다. (converters.convert_to_pdf 위임)"""
    from .converters import convert_to_pdf

    return convert_to_pdf(hwp_path, output_pdf_path)
